...
public abstract class AbstractAltarEntity extends LivingEntity {
    ...
    public static final DataParameter<ItemStack> HELD_OFFERING = EntityDataManager.defineId(FamilyAltarEntity.class, DataSerializers.ITEM_STACK);

    private final ImmutableList<ItemStack> armorItems = ImmutableList.of();
    public long lastHit;
    private int stateTime;

    public AbstractAltarEntity(EntityType<? extends AbstractAltarEntity> entityType, World world) {
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(ALTAR_STATE_ID, FamilyAltarEntity.AltarState.NEUTRAL.ordinal());
        this.entityData.define(HELD_OFFERING, ItemStack.EMPTY);
    }
    ...
    public static void setAltarState(CompoundNBT tag, AbstractAltarEntity altarEntity) {
    ...
    public abstract List<Item> getValidOffering();
    ...
    public ItemStack getHeldOffering() {
        return this.entityData.get(HELD_OFFERING);
    }

    public void setHeldOffering(@Nonnull ItemStack itemStack) {
        this.entityData.set(HELD_OFFERING, itemStack);
    }
            ...
            if (state != AltarState.NEUTRAL) {
                ...
                if (this.stateTime <= 0) {
                    if (state == AltarState.HAPPY) {
                        this.showOfferingVanishParticles();
                    }
    ...
    public void addAdditionalSaveData(CompoundNBT compoundNBT) {
        ...
        if (!this.getHeldOffering().isEmpty()) {
            this.getHeldOffering().save(cakeTag);
            compoundNBT.put("HeldOffering", cakeTag);
        }
    ...
    public void readAdditionalSaveData(CompoundNBT compoundNBT) {
        ...
        if (compoundNBT.contains("HeldOffering", Constants.NBT.TAG_COMPOUND)) {
            CompoundNBT offeringTag = compoundNBT.getCompound("HeldOffering");
            this.setHeldOffering(ItemStack.of(offeringTag));
        }
        if (compoundNBT.contains("AltarState", Constants.NBT.TAG_ANY_NUMERIC)) {
            int state = compoundNBT.getInt("AltarState");

            if (state > FamilyAltarEntity.AltarState.values().length || state < 0) {
                StarvingTotem.LOGGER.error("Tried to load altar state of Family Altar, but the read state ID was not valid. Received invalid state ID \"{}\"; must range from 0 to \"{}\".", state, FamilyAltarEntity.AltarState.values().length);
    ...
    public ActionResultType interactAt(PlayerEntity player, Vector3d vec3d, Hand hand) {
        ItemStack heldCake = this.getHeldOffering();

        if (player.isShiftKeyDown()) {
            this.setHeldOffering(ItemStack.EMPTY);
        ...
        if (!heldCake.isEmpty()) {
            ...
            if (this.getValidOffering().contains(itemStack.getItem())) {
                AltarState state = this.getAltarState();
                ...
                if (state == AltarState.HAPPY) {
                    ...
                    this.setHeldOffering(itemStack.copy());
                    itemStack.shrink(1);
    ...
    private static ItemStack writeDataToStack(AbstractAltarEntity entity) {
        ItemStack stack = entity.getPickedResult();
    ...
    private void showOfferingVanishParticles() {
        if (this.level instanceof ServerWorld) {
            ServerWorld serverWorld = (ServerWorld) this.level;
    ...
    public abstract Supplier<BlockState> getBreakParticleBlock();

    public abstract Supplier<SoundEvent> getBreakSound();

    private void showBreakingParticles() {
        if (this.level instanceof ServerWorld) {
            ((ServerWorld)this.level).sendParticles(new BlockParticleData(
                            ParticleTypes.BLOCK,
                            this.getBreakParticleBlock().get()),
                    this.getX(),
    ...
    private void playBrokenSound() {
        this.level.playSound(null, this.getX(), this.getY(), this.getZ(), this.getBreakSound().get(), this.getSoundSource(), 1.0F, 1.0F);
    }
}
